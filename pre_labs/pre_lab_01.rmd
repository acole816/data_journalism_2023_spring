...
**Answer**  55,632 observations (rows), 7 variables (columns)
...
**Answer**  id data type = chr, seats data type = dbl
...
**Answer** Line 128 is use the umd_course dataframe, Line 129 is saying group by each term, Line 130 is saying to summarize the data frame into one value , Line 131 says to create a column named "count classes" and to add up all the classes within a given term.  
...
**Answer** Now, we have another "and then..." and a final line is ordering the 'count_classes' column we created in descending order, showing the term with the greatest number of classes first, descending downwards. 
...
**Answer** Now we are grouping by term and department, counting how many classes are in a given department for a certain semester.
...
Compared to the previous codeblock, what has changed and how did that affect the result? 
**Answer** Now we are seeing the term, the number of classes in that term, and are sorting in descending order by the total seats, which displays the terms with the greatest amount of total seats first. 
...
**Answer** Mean number of seats and median number of seats have now been added, but we are still sorting by total setas in descending order. 
...
**Answer** Now instead of grouping by term, we are grouping by department. Additionally, we've now added two new columns, minimum and maximum number of seats.   

### Task 12: Questions

Task 15? Nice trick question.

No code here; I want you to provide three questions that this data suggests to you. What would you like to ask it?

**Answer**

1.  What departments are trending upward, based on number of total classes or seats from previous years? Which of them are trending downward?

2.  Which departments have the biggest and smallest students to total classes ratio? 

3.  Classes listed as NA description, NA teacher and 0 seats just never took place...? Why are these included in the data set? Could we remove these data points? Will they effect conclusions we make when asking other questions about the dataframe?
