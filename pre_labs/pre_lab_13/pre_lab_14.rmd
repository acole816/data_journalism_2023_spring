...
**Answer** Data frame showing states, who won the Presidential election in that state in 2016, Trump's voter share percentage in that state, percent of small businesses in that state receiving PPP loans and the amount of that loan per employee. 
```{r}
...
**Answer** Scatter plot showing the mostly positive correlation between states that voted more for Trump in 2016 and states how received a higher percentage of PPP loans per business. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Scatter Plot showing slight positive correlation between states that voted more for Trump in 2016 and amount of PPP loan per employee in those states. 
```{r}
...
**Answer** Shows a positive correlation between Trump's voter percentage in a state and that states percentage of small businesss PPP loans received. Also shows a verey slight positive correlation between rump's voter percentage in a state and that states $ amount of a loan per employee. 
```{r}
reveal_data %>%
...
**Answer** Shows us the p-value, which shows us if something is random chance or if things are actually correlated. This small p-value of 8.607e-05 is a very small p-value, indicating the relationship between Trump's voter percentage in a state and that states percentage of businesses that received PPP loans is not a random occurrence, but rather statistically valid. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Higher p-value here (above .05) indicates that this is not statisitcally valid and very well could ahve happened by chance. 
```{r}
...
**Answer** Shows states that voted more for Trump and their mean PPP loan $ amount per employee and mean amount of businesses in that state that recieved a PPP loan amount. The same is shown for states that voted for Clinton. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** Shows us the p-value between the mean amount per employee for states Clinton won and the same for states Trump won. Because the p-value is so high (.2295), we can infer that the difference between these 2 is jsut randonm chance and isn't statistically significant. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Same things as task 9, but now we are looking at the correlation between states that voted for Trump and the percentage of that state's small businesses who received PPP loans, as well as states that voted for Clinton. Here, we have a much lower p-value (.002643) which indicates this correlation is statistically significant. This can also be seen in the small range of the 95% confidence interval. 
```{r}
