...
**Answer** "Warning: One or more parsing issues, call `problems()` on your data frame for details, e.g.:" 
When I call 'problems()', it shows the rows and columns where the computer (by the way, what word do I use here...? What the "code" expected? "Computer"? "R"?) expected a certain value and what is actually there.

NOTE: Don't use guess_max=10, not enough.
...
**Answer** Code is filtering by zip codes that have less then 5 digits and counting how many times each of those  individual > 5 digit zip codes appear.
...
**Answer** There are 1 uncategorized transactions in 2010 and 2011. 2013 also has 50 transactions filtered under "L" instead of Loan. 2017 has a transaction filtered under "Contract" and also has a NA transaction like the ones in 2010 and 2011. 
...
**Answer** All transactions in the data frame the are arranged from least to greatest according to the amount of the transaction. 
...
**Answer** Changes existing column names with all lowercase and replaces spaces with underscores. 
...
**Answer** There is now a new column that shows the first 5 digits of every Zip Code called 'Zip5'. 
...
**Answer** Zip Code 21202 with $648,069
