...
**Answer** The new column here that we haven't worked with before is the 'geometry' column, which shows the shape and cooridnates of each house. 
...
**Answer** The geometry column is different then any column we've worked with before because there is a little search icon next to each observation. You can click on this search bar and it pulls up more data and coordinates for that row. 
...
**Answer** Makes a zipcode map of MD. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We have installed and loaded the 'tigirls' package.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** There is 18 different column, including the new geometry column we haven't seen before today. This is a database of all counties in the U.S. 
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This is a data frame taken form the last one that just has MD counties, instead of all U.S. counties. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This is a county map of MD. What is interesting about this map is it doesn't look like the Chesapeake Bay exist. This is because Eastern shore counties boundary lines extend all the way to the western side of the Chesapeake Bay. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This is the zip code map on top of the county map for MD. It is important to have the order of our code have the least specific map/thing first, and increasingly become more specific as we go down.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** This data frame shows us foreclosure data for zip codes in MD during the month of March in 2023. One thing I noticed is this data frame has 547 rows, whereas the MD Zips data frame only has 526 rows. This means their our zipcodes in this data frame outside of MD or that were typed in incorrectly. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer** This code is joining the MD zip code data frame with the foreclosure's in march '23 dataframe over the common column they share, zip codes. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** A map of MD showing areas with the highest numbers of foreclosure in March '23. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This codeblock gave us 2 in data frames. md_county_population and md_county_notices. md_county_population is showing the population per county in MD, with no MOE. The md_county_notice shows us the number of notices of foreclosure per zip code in MD in March '23. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This is the same thing as the previous md_county_population data frame from the previous codeblock, but now we have the geometry column we had in the data frames from the beginning of the pre_lab. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This shows us the 2 data frames we are working with, md_county_population and md_county_notice. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We are combining the md_county_population data with the md_county_notice data. Because we want to join on county names, but the column names are different and formatted differently in both data frames, we have to mutate one of the column names to be formatted in the same way as the other, so we can join the data frames together. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This codeblock is actually joining the 2 data frames together after we have mutated the column names and formatting of each to match each other. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This code block is calculating the foreclosure notice rate in each county per 1000 people in its population. The data is then sorted from highest percent to lowest. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This is a county map that shows the percentage rate of forclosure notices in each county. The map is colered in different shades of blue. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The same map from the previous codeblock, but now ther is a new color scheme that shows the data in a more clear way then the blue map. 
...
**Answer** Same map as the previous two, but again this map does a slightly better job then the previous using distinct colors that help the reader more clearly identify the different percentage levels on the map. 
