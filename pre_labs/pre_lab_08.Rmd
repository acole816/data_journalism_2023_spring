...
**Answer** 3822 rows and 4 columns
...
**Answer** First we are creating a new dataframe from our existing one that will count the nuber of calls for each month. Then, we are using mutate to add a column called 'month', which is taking the month from the 'date' column. label=TRUE is making the month a string oppose to a numeric value. Then we are grouping by month, and using the summarize function and arrange function, in descending order, to show the months with the highest total number of calls first.
...
**Answer** A big blank gray sqaure. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** A black and gray bar chart showing the number of calls in each month. 
...
**Answer** Now instead of the months being ordered in chronological order, they are ordered from least number of total calls per month to greatest. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Now the bar graph is fixed horizontally opposed to vertically. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The background is now white instead of gray. 
...
**Answer** Now the chart has a blueish background.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** We gave the chart a title and caption and changed the name and changed the names of the x and y axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** A list of dates followed by the number of calls on those dates.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** A line chart with a gray background with total calls as the y-axis and dates on the x-axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Same thing as last but now the x-axis has so many dates they are all overlapping ans impossible to read.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now the dates on the x-axis aren't overlapping as much as they previously were, showing now diagnolly to try and give each one more space. Still challenging to read. 
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
